...
export function Header({ onAdd, onShare, onXP }: HeaderProps) {
  ...
  const xpInfo = isTeam && state.profile.live ? xpProgress(state.profile.xp) : null;

  return (
    <header className="ff-lattice sticky top-0 z-40 border-b border-gold-dk bg-gradient-to-b from-onyx to-[#0A0A0B] text-cream">
      <div className="mx-auto flex max-w-[1180px] items-center gap-3 px-3.5 py-2.5">
        <Coin size={30} tier={isTeam ? state.profile.tier : 'gold'} />
        ...
        {state.role && (
          <nav className="hidden desk:ml-auto desk:flex desk:items-center desk:gap-0.5">
            ...
            <button className="ml-2.5" onClick={onXP} aria-label="Your FireFlair XP">
              <Coin size={30} tier="gold" />
            </button>
            <button
              onClick={isTeam ? onShare : onAdd}
              aria-label={isTeam ? 'Share your FireFlair card' : 'Add FireFlair'}
        ...
        {state.role && (
          <button
            ...
            onClick={() => dispatch({ type: 'ROLE', role: isTeam ? 'staff' : 'team' })}
          >
      ...
      {(xpInfo || basket) && (
        <div className="hidden desk:mx-auto desk:flex desk:max-w-[1180px] desk:items-center desk:gap-4 desk:border-t desk:border-gold/18 desk:px-3.5 desk:py-[9px]">
          {xpInfo && (
            <button onClick={onXP} className="flex min-w-0 flex-1 items-center gap-2.5" aria-label="Your FireFlair XP">
              <Coin size={24} tier={xpInfo.currentTier} />
              <span className="min-w-0 flex-1">
                <span className="mb-[3px] flex justify-between text-[10px] tracking-[0.1em] text-gold-lt uppercase">
                  <span>{xpInfo.xp} XP</span>
                  <span className="text-cream/55">{xpInfo.next ? `Next: ${xpInfo.next.label}` : 'Top tier'}</span>
                </span>
                <span className="ff-xptrack h-[5px]">
                  <span className="ff-xpfill" style={{ width: `${xpInfo.pct}%` }} />
                </span>
              </span>
            </button>
          )}
          {basket && (
            <div className="flex items-center gap-2.5">
              <span className="text-[11px] tracking-[0.1em] text-gold-lt uppercase">{state.selection.length} selected</span>
              <button onClick={() => dispatch({ type: 'CLEAR_SELECTION' })} className="text-[11px] tracking-[0.08em] text-cream/60 uppercase">
                Clear
              </button>
              <button
                onClick={() => dispatch({ type: 'TAB', tab: 'home', route: 'shiftNew' })}
